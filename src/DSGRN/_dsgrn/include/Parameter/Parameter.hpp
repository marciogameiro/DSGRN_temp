...
regulator ( uint64_t variable, uint64_t threshold ) const {
  ...
  // Here we are assuming that we blowup every self-edge (not
  // just repressing self-edges). Need to change the code to
  // count only self-repressing thresholds if these are the
  // only ones we are blowing up.

  // The variable "threshold" correspond to the domain threshold.
  // In case of self-edge threshold blowup this threshold may not
  // be one of the out-edge thresholds. So we count the number of
  // blowup thresholds less than or equal to "threshold" and subtract
  // it from "threshold" to get the corresponding out-edge threshold.

  // Get number of blowup thresholds less or equal "threshold"
  auto num_blowup_thresholds = [&]() {
    uint64_t num_blowup_thresh = 0;
    for ( uint64_t thresh = 0; thresh < num_outedges; ++ thresh ) {
      ...
      // Only count blowup thresholds less than or equal to "threshold"
      // The blowup threshold corresponding to thresh has not been
      // counted yet, so if "threshold" happens to be that blowup
      // threshold it will be caught on the next loop iteration
      // (hence the >= condition instead of just ==).
      if ( thresh + num_blowup_thresh >= threshold ) {
        return num_blowup_thresh;
      }
      if ( variable == target ) { // If self-edge
        num_blowup_thresh += 1;   // Increment counter
      }
    }
    return num_blowup_thresh;
  };

  // Subtract number of blowup thresholds to get out-edge threshold
  uint64_t out_edge_threshold = threshold - num_blowup_thresholds();
...
instance ( uint64_t variable, uint64_t threshold ) const {
  uint64_t num_outedges = network () . outputs ( variable ) . size ();
  // Return 0 if no out-edges
  if ( num_outedges == 0 ) {
    return 0;
  }

  // Variable being regulated on threshold
  uint64_t regulated_var = regulator (variable, threshold);

  // Get number of edges from variable to regulated_var
  // for all out-edges less than or equal threshold
  uint64_t num_multi_edges = 0;
  uint64_t num_blowup_thresh = 0;
  for ( uint64_t thresh = 0; thresh < num_outedges; ++ thresh ) {
    uint64_t out_index = order () [ variable ] ( thresh );
    uint64_t target = network () . outputs ( variable ) [ out_index ];
    if ( target == regulated_var ) {
      num_multi_edges += 1; // Increment edge counter
    }
    if ( variable == target ) { // If self-edge
      num_blowup_thresh += 1;   // Increment counter
    }
    if ( thresh + num_blowup_thresh >= threshold ) {
      // Instance is num edges minus 1
      return num_multi_edges -1 ;
    }
  }
  return num_multi_edges - 1;
}
